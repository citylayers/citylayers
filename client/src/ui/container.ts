...
class ToProjects extends BaseComponent {
    ...
    constructor(parent: string, id: string) {
        ...
        this.clickHandler = () => {
            const CityLayersPanel = (window as any).CityLayersPanel;
            if (CityLayersPanel) {
     ...
     */
    static activate(on: boolean): void {
        let elements = Array.from(document.getElementsByClassName(
            ClassName.CATEGORYPANEL_LABEL
        )).filter((e: any) => e.innerText == ToProjects.content);

        if (elements.length > 0) {
            (elements[0] as HTMLElement).style.display = on === false ? DisplayStyle.NONE : DisplayStyle.FLEX;
        }
    }
    ...
    static hide(): void {
        ...
        if (elements.length > 0) {
            (elements[0] as HTMLElement).style.display = DisplayStyle.NONE;
        }
    }
}
...
class SubcategoryTagContainer extends BaseComponent {
     ...
     */
    static getByCategory(category: string): HTMLElement | null {
     ...
     */
    static addLabel(category: string, label: any, togglable: boolean = false): void {
        togglable = false;
...
class SubcategoryTag extends BaseComponent {
    ...
    constructor(parent: string, tag: any) {
    ...
    toggle(): void {
        ...
        if (!parentElement || !parentElement.parentElement || !parentElement.parentElement.parentElement) return;
        ...
        const ControllerMultiCategorical = (window as any).ControllerMultiCategorical;
        const ConfigPanel = (window as any).ConfigPanel;

        if (!ControllerMultiCategorical || !ConfigPanel) return;

        let container = ControllerMultiCategorical.getByCategory(category);
        let existing_ids = Array.from(container.children).map((el: any) => el.id);
     ...
     */
    initiate(togglable: boolean = true): void {
        super.initiate();
